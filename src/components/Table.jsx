...
export default function Table({ columns, rows, isLoading, emptyMessage = 'No records found.' }) {
  if (isLoading) {
    return (
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-surface" role="status" aria-label="Loading">
  ...
  if (!rows?.length) {
    return (
      <div className="rounded-lg border border-dashed border-slate-300 bg-surface py-10 text-center text-sm text-slate-500">
        {emptyMessage}
      </div>
    );
  }
